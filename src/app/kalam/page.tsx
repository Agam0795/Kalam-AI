...
export default function Page() {
  ...
  const handleGenerate = async () => {
    ...
    setOutput("");
    try {
      // Try hitting the app's generation endpoint if present
      ...
      if (!text) throw new Error("No text returned by API");
      setOutput(String(text));
    } catch (e: any) {
      // Graceful fallback demo text so the UI remains functional
      const fallback = `Here is a sample Kalam AI response based on your prompt:\n\n${prompt}\n\n— Kalam AI (demo mode)`;
      setOutput(fallback);
      setError(e?.message ? String(e.message) : "Failed to generate. Showing demo output.");
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    // subtle mount animation trigger
  ...
  return (
    <div className="page">
      ...
      <main>
        ...
        {/* Demo */}
        <section id="demo" className="section demo" aria-labelledby="demo-heading">
          <div className="container">
            <h2 id="demo-heading" className="section-title">Try Kalam AI</h2>
            <p className="section-sub">Enter a prompt and experience thoughtful, human-grade output.</p>
            ...
            <div className="demo-grid">
              <div className="demo-input card">
                ...
                <p className="hint">Tip: Be specific about tone, audience, and length.</p>
              </div>

              <div ref={outputRef} className="demo-output card">
                <div className="output-header">
                  <h3 className="card-title">Output</h3>
                  <div className="output-actions">
                    <button className="btn small" onClick={handleCopy} disabled={!output} title="Copy">
                      {copied ? "Copied" : "Copy"}
                    </button>
                  </div>
                </div>
                <div className={`output-body ${loading ? "is-loading" : ""}`} aria-live="polite">
                  {loading && <div className="skeleton" />}
                  {!loading && output && (
                    <pre className="pre">{output}</pre>
                  )}
                  {!loading && !output && (
                    <div className="placeholder">Your results will appear here.</div>
                  )}
                </div>
              </div>
            </div>
      ...
      <style jsx global>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .orb { position: absolute; border-radius: 999px; filter: blur(16px); opacity: 0.66; transform: translateY(calc((1 - var(--mounted)) * 20px)); transition: transform 600ms ease 60ms, opacity 600ms ease 60ms; }
        .orb-1 { width: 220px; height: 220px; right: 12%; top: 8%; background: radial-gradient(100% 100% at 50% 50%, #8ec9ff 0%, rgba(142,201,255,0) 70%); }
        .orb-2 { width: 320px; height: 320px; left: 6%; bottom: -10%; background: radial-gradient(100% 100% at 50% 50%, #b7e1ff 0%, rgba(183,225,255,0) 70%); }
        .panel { position: absolute; inset: 10% 12%; border-radius: 18px; background: rgba(255,255,255,0.7); backdrop-filter: blur(6px); border: 1px solid #d7e7fa; box-shadow: 0 14px 40px rgba(12, 68, 140, 0.12); overflow: hidden; transform: translateY(calc((1 - var(--mounted)) * 24px)); transition: transform 640ms ease; }
        .panel-row { height: 33.33%; border-bottom: 1px dashed #e5f0ff; }
        ...
        .output-body { min-height: 220px; border: 1px dashed #cfe1f4; background: #fbfdff; border-radius: 12px; padding: 14px; position: relative; }
        .pre { white-space: pre-wrap; margin: 0; color: #11365f; }
