...
import { Send, Bot, User, RefreshCw, Settings, MessageSquare, Trash2, Zap, Globe } from 'lucide-react';
...
export default function EnhancedChatBot() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const endpoint = settings.provider === 'gemini' ? '/api/chat' : '/api/chat-alternative';
      ...
      const requestBody = {
        ...
        ...(settings.provider === 'alternative' && { model: settings.model })
      };
  ...
  const getProviderIcon = () => {
    return settings.provider === 'gemini' ? <Globe className="h-4 w-4" /> : <Zap className="h-4 w-4" />;
  };

  const getProviderName = () => {
    if (settings.provider === 'gemini') return 'Google Gemini';
    const selectedModel = ALTERNATIVE_MODELS.find(m => m.id === settings.model);
    return selectedModel ? selectedModel.name : 'Alternative API';
  };
    ...
    <div className="max-w-4xl mx-auto h-screen flex flex-col bg-white">
        ...
        <div className="bg-gray-50 border-b p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              ...
              <div className="text-xs text-gray-500">Free, reliable</div>
            </button>
            <button
              onClick={() => setSettings(prev => ({ ...prev, provider: 'alternative' }))}
              className={`p-3 rounded-lg border-2 transition-colors ${
                settings.provider === 'alternative'
                  ? 'border-purple-500 bg-purple-50 text-purple-700'
                  : 'border-gray-200 hover:border-gray-300'
              }`}
            >
              <Zap className="h-5 w-5 mx-auto mb-1" />
              <div className="text-sm font-medium">Alternative APIs</div>
              <div className="text-xs text-gray-500">Multiple models</div>
            </button>
          </div>
              ...
              {settings.provider === 'alternative' && (
                <div>
                  ...
                  <select
                    ...
                    title="Select the AI model to use"
                  >
        ...
        {messages.length === 0 ? (
          <div className="text-center text-gray-500 mt-20">
            ...
            <div className="mt-4 text-xs text-gray-400">
              Currently using: {getProviderName()}
            </div>
          </div>
        ) : (
      ...
      <div className="border-t bg-white p-4">
        ...
        <div className="text-xs text-gray-500 mt-2 text-center">
          Press Enter to send • Shift+Enter for new line • Using {getProviderName()}
        </div>
      </div>
    </div>
  );
